...
void find_cur(){
    ...
    GetDiskFreeSpaceEx(name, NULL, NULL, &free_memory); //디스크 용량 정보 받기
...
void find_target_dir(char *argv[]){
    ...
    printf("\n %s\\%s 디렉터리\n\n", getcwd(name, 100), argv[1]);//폴더면 경로에 추가해서 출력
    GetDiskFreeSpaceEx(name, NULL, NULL, &free_memory); //디스크 용량 정보 받기
...
void find_target_file(char *argv[]){
    ...
    printf("\n %s 디렉터리\n\n", getcwd(name, 100)); //현위치 출력
    GetDiskFreeSpaceEx(name, NULL, NULL, &free_memory);
